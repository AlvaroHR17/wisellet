...
public class ListTransactionTypesController {
    ...
    public ListTransactionTypesReferenceData list() {
        List<IncomeTypeDto> incomeTypes = incomeTypeDao.findAll().stream()
                .map(incomeTypeFactory::getDto)
                .collect(Collectors.toList());

        List<FixedExpenseTypeDto> fixedExpenseTypes = fixedExpenseTypeDao.findAll().stream()
                .map(fixedExpenseTypeFactory::getDto)
                .collect(Collectors.toList());

        List<VariableExpenseTypeDto> variableExpenseTypes = variableExpenseTypeDao.findAll().stream()
